...
export default function VehicleCard({ vehicle, onToggleFavourite, isFavourite, onToggleCompare, isComparing }) {
  const primaryImage =
  ...
  return (
    <div className="group bg-charcoal-900 border border-charcoal-700 rounded-md overflow-hidden hover:border-accent/50 transition-colors">
      <div className="relative aspect-[4/3] overflow-hidden">
        ...
        <span
          className={`absolute top-3 left-3 text-xs font-medium px-2.5 py-1 rounded-sm ${AVAILABILITY_STYLES[vehicle.availability]}`}
        >
        ...
        <button
          ...
          className={`absolute top-3 right-3 w-8 h-8 rounded-full flex items-center justify-center backdrop-blur-sm transition-colors ${
            isFavourite ? "bg-accent text-charcoal-950" : "bg-charcoal-950/60 text-ivory hover:bg-charcoal-950/80"
          }`}
        >
          ♥
        </button>
      ...
      <div className="p-4">
        <div className="flex items-start justify-between gap-2">
          ...
          <span className="text-xs text-ivory/50 whitespace-nowrap pt-1">{vehicle.year}</span>
        </div>

        <p className="text-accent font-semibold mt-1">{formatPrice(vehicle.price)}</p>
        ...
        <div className="flex items-center gap-2 mt-4">
          ...
          <a
            href={whatsappLink(`Hi, I'm interested in the ${vehicle.year} ${title} (Stock #${vehicle.stockNumber}).`)}
            target="_blank"
          ...
          <button
            ...
            className={`w-9 h-9 flex items-center justify-center rounded-sm text-xs transition-colors ${
              isComparing ? "bg-accent text-charcoal-950" : "bg-charcoal-800 text-ivory/70 hover:bg-charcoal-700"
            }`}
